...
class ReactionPathVisualizer:
    ...
    def __init__(self, img_size: Tuple[int, int] = (300, 300), 
                 font_size: int = 12, 
                 padding: int = 20):
        """
        初始化可视化器
    ...
    def _create_arrow_image(self, text: str) -> Image.Image:
        ...
        text_start_y = arrow_center_y - len(text_lines) * 6 - 10
        for i, line in enumerate(text_lines):
            bbox = draw.textbbox((0, 0), line, font=font)
            text_width = bbox[2] - bbox[0]
            text_x = (arrow_width - text_width) // 2
            draw.text((text_x, text_start_y + i * 12), line, fill='black', font=font)
    ...
    def _create_arrow_with_reagent(self, arrow_text: str, reagent_img: Image.Image) -> Image.Image:
        """创建带有试剂的反应箭头"""
        arrow_width = 200
        arrow_height = reagent_img.height + 60  # 为箭头留出空间
        ...
        # 绘制箭头
        arrow_y = reagent_y + reagent_img.height + 10
        arrow_start_x = 30
        arrow_end_x = arrow_width - 30
        ...
        for i, line in enumerate(text_lines):
            ...
            text_width = bbox[2] - bbox[0]
            text_x = (arrow_width - text_width) // 2
            draw.text((text_x, text_y + i * 12), line, fill='black', font=font)
    ...
    def _combine_step_components(self, components: List[Image.Image]) -> Image.Image:
        ...
        # 计算总宽度和最大高度
        total_width = sum(img.width for img in components) + (len(components) - 1) * 20
    ...
    def _combine_images_vertically(self, images: List[Image.Image]) -> Image.Image:
        ...
        total_height = sum(img.height for img in images) + (len(images) - 1) * 30
