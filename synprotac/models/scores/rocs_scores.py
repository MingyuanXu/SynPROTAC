...
class ShapeScore(BaseScore):
    """Compute shape similarity scores using ROCS."""
    def __init__(self, shape_w: float = 1.0, color_w: float = 1.0, cff_path: str = '', sdf_path: str = '', sim_measure: str = 'Tanimoto'):
        self.rocs_model = Rocs_scorer(shape_w=shape_w, color_w=color_w, cff_path=cff_path, sdf_path=sdf_path, sim_measure=sim_measure)
    
    def compute_scores(self, mols: List[Chem.Mol]) -> torch.Tensor:
        smiles = [Chem.MolToSmiles(mol) if mol else 'None' for mol in mols]
        scores = self.rocs_model._calculate_omega_score(smiles)
